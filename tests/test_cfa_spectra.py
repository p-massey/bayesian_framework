...
# --- CONFIGURATION ---
SPECTRA_DIR = "/Users/pxm588@student.bham.ac.uk/Desktop/snid/cfa_pipeline/all_spectra_dereddened_snidded_supersnid"
PARAM_FILE = os.path.join(SPECTRA_DIR, "cfasnIa_param.dat")
NLIVE = 100
MODEL_NAME = 'salt3'
...
def load_flm_spectrum(file_path):
    # Handles .flm and .dat (assumes 2 or 3 columns)
...
def run_test():
    ...
    results = []
    # User requested only .flm files
    flm_files = sorted([f for f in os.listdir(SPECTRA_DIR) if f.endswith('.flm')])
    
    print(f"Found {len(flm_files)} .flm files in {SPECTRA_DIR}")

    for i, filename in enumerate(flm_files):
        file_path = os.path.join(SPECTRA_DIR, filename)
        
        # Parse SN name and date
        ...
        # Ignore spectra without a valid MJD_max
        if p['mjd_max'] > 90000:
            continue
        ...
        # User requested filtering for true_age from -20 to 50 days
        if not (-20 <= true_age <= 50):
            continue
        ...
        # Common wavelength mask for SALT3
        ...
        # 2. Nuisance Fit (marginalized x0)
        try:
    ...
    df.to_csv("outputs/csvs/cfa_test_results.csv", index=False)
    ...
    ax1.set_title(f'Method Comparison on CfA Spectra ({len(df)} files)')
    ax1.legend()
    ...
    plt.tight_layout()
    plt.savefig("outputs/plots/cfa_age_comparison.png")
    print(f"\nPipeline complete. Processed {len(df)} matched files.")
    print("Results saved to outputs/csvs/cfa_test_results.csv and outputs/plots/cfa_age_comparison.png")
