...
def log_likelihood_full(t, params, wavelength, flux, flux_err, model_name, redshift):
    ...
    try:
        ...
        # FIX: Must return -np.inf to prevent rslice from hanging
...
def log_likelihood_nuisance(t, params, wavelength, flux, flux_err, model_name, redshift):
    ...
    try:
        ...
        x0_best = num / den
        if x0_best <= 0: return -np.inf  # FIX: Enforce valid x0 space properly
...
def fit_full(wavelength, flux, flux_err, model_name='salt3', redshift=None):
    ...
    ll = partial(log_likelihood_full, params=params, wavelength=wavelength, flux=flux, flux_err=flux_err,
                 model_name=model_name, redshift=redshift)

    # FIX: Removed redundant and crash-prone dynesty checkpointing
...
def process_single_file(filename, sn_params):
    ...
    for method, fit_func in [('full', fit_full), ('nuis', fit_nuisance)]:
        try:
            ...
            if method == 'nuis':
                x0_list = []
                # FIX: Cap samples at 500 to dramatically speed up this nuisance loop
                subset_samples = samples[np.random.choice(len(samples), min(500, len(samples)), replace=False)]
...
def run_test():
    ...
    # FIX: Pre-load model in main process to safely seed Astropy cache
    print(f"Pre-loading {MODEL_NAME} model to prevent race conditions...")
    _ = sncosmo.Model(source=MODEL_NAME)
